...
void lcd_test(void)
{
	...
	Gui_DrawFont_GBK16(16,10,YELLOW,GRAY0,lcd_str);	
}

void Gui_DrawFont_GBK16_FullLCD(uint16_t fc, uint16_t bc, uint8_t *s)
{
	unsigned char i,j;
	unsigned short k,x0;
...
void lcd_printf(const char *fmt /*format*/, ...)
{
	va_list ap;
	uint8_t lcd_str[256] = {0};
	va_start(ap, fmt);
	vsprintf((char *)lcd_str, fmt, ap);
	Lcd_Clear(GRAY0);
  Gui_DrawFont_GBK16_FullLCD(BLUE,GRAY0,lcd_str);
	va_end(ap);
}
